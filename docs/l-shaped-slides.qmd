...
Convert the input to the standard format (automatically):
...
We can utilize the built-in Benders decomposition (for example, in Cplex) better by annotating the model:
...
# Variables
# Q - order quantity
# z[i] - total cost for scenario i
cpx.variables.add(names=["Q"])
for i in range(len(demands)):
    cpx.variables.add(obj=[probability[i]], names=[f"z_{i}"])

# Set Benders annotations
cpx.variables.set_annotations("BendersAnnotation", [0], [1])  # Q is in master problem
for i in range(len(demands)):
    cpx.variables.set_annotations("BendersAnnotation", [i + 1], [-1])  # z[i] in subproblem

# Constraints
for i, demand in enumerate(demands):
    # Subproblem constraints
    expr = cplex.SparsePair(ind=["Q", f"z_{i}"], val=[C_o - C_h, 1])
    cpx.linear_constraints.add(lin_expr=[expr], senses=["G"], rhs=[-C_h * demand])

    expr = cplex.SparsePair(ind=["Q", f"z_{i}"], val=[C_o + C_p, 1])
    cpx.linear_constraints.add(lin_expr=[expr], senses=["G"], rhs=[C_p * demand])
```
