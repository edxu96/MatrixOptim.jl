...
In contrast, the corresponding deterministic formulation assumes the calculation of $\mathscr{Q}(\overline{\mathbf{y}})$ to be easy, so there is no the second stage variables in that formulation.
