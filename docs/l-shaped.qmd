...
For the sake of clearity, $\overline{\mathbf{y}}$ can represent a given value of $\mathbf{y}$:
...
$\mathscr{Q}(\overline{\mathbf{y}})$ can be easily calcuated in the corresponding deterministic formulation, but it will be obtained iteratively in the L-shaped decomposition, together with iteratively added optimality cuts and feasibility cuts.
...
$$
\begin{aligned}
    \max_{\mathbf{u}_s} \quad & \left(\mathbf{h}_s - \mathbf{T}_s \overline{\mathbf{y}} \right)^\top \mathbf{u}_s \\
