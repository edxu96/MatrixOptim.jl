...
let
    ...
    while (boundUp - boundLow > epsilon && timesIteration < 10)
        ...
        let
            ...
            @variable(sub, u[1:s] >= 0)
            @objective(sub, Max, - (demand[1] + yBar) * u_1 - (demand[2] + yBar) * u_2 - (demand[3] + yBar) * u_3 -
                                 (demand[4] + yBar) * u_4 - (demand[5] + yBar) * u_5 - (demand[6] + yBar) * u_6 -
                                 (demand[7] + yBar) * u_7 - (demand[8] + yBar) * u_8 - (demand[9] + yBar) * u_9 -
                                 (demand[10] + yBar) * u_10)
